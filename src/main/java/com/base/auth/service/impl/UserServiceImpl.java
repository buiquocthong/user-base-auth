...
public class UserServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String userId) {
        log.info("loadUserByUsername called with userId: {}", userId);
        log.info("Call stack: ", new RuntimeException("Call stack"));

        Account user = accountRepository.findAccountByUsername(userId);
        if (user == null) {
            user = accountRepository.findAccountByEmail(userId);
            if (user == null)
    ...
    public OAuth2AccessToken getAccessTokenForCustom(ClientDetails client, TokenRequest tokenRequest, String email, String password, AuthorizationServerTokenServices tokenServices) throws GeneralSecurityException, IOException {
        Map<String, String> requestParameters = new HashMap<>();
        requestParameters.put("grantType", SecurityConstant.GRANT_TYPE_PASSWORD);
        String clientId = client.getClientId();
        boolean approved = true;
    ...
    public OAuth2AccessToken getAccessTokenForCustomTypeEmail(ClientDetails client, TokenRequest tokenRequest, String email, String password, AuthorizationServerTokenServices tokenServices) throws GeneralSecurityException, IOException {
        Map<String, String> requestParameters = new HashMap<>();
